...
export const ModalAnime: React.FC<Props> = ({
	open,
	setOpen,
	inputStatus,
	setInputStatus,
	...
	const handleSave = () => {
		...
		if (currentUser) {
			...
			if (inputStatus === "Completed" && typeof item.episodes === "number")
			...
			let episodesRef = firestore
				...
				.collection(Collections.anime)
				.doc(item.title);
			if (inputEpisode != null)
				batch.set(episodesRef, { episode: inputEpisode });
			if (inputStatus === "Completed") {
				if (typeof item.episodes === "number") setInputEpisode(item.episodes);
